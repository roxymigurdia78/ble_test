// parent.c (座標正負反転修正版, WiringPi BCM 統一)
...
// プロトコル時間定義
#define P_MAG_TIME_SEC   30  // 親機電磁石ON時間
#define P_SCAN_TIME_SEC  10  // 親機が子機レポートを受信する時間 (P-TXフェーズ後)
#define C_MT_TIME_SEC    10  // MT広告送信時間
#define C_ACT_TIME_SEC   20  // 子機電磁石ON時間/ホールセンサー検知時間
#define C_TX_TIME_SEC    10  // 親機が子機レポートを受信する時間 (C-ACTフェーズ後)
#define COOLDOWN_SEC      5  // クールダウン時間
...
const int BCM_GPIO_PIN = 20; // 親機の電磁石も BCM 20 を使用
// ==========================================================
// キーリスト/状態管理 構造体
// ==========================================================
typedef struct {
...
// 子機レポートログ出力制御用変数 (ノードアドレスごとにログ出力フラグを管理)
...
// ==========================================================
// GPIO/I2C/ADC/KeyEntry 関数定義
// ==========================================================
// ダミーまたは単純なI/O関数の再定義 (Sysfs依存部分を削除)
static int readInt(const char *path) { FILE *f = fopen(path, "r"); if (!f) return -1; int v; if (fscanf(f, "%d", &v) != 1) { fclose(f); return -1; } fclose(f); return v; }
static int writeFile(const char *path, const char *value) { FILE *f = fopen(path, "w"); if (!f) return -1; if (fprintf(f, "%s", value) < 0) { fclose(f); return -1; } fclose(f); return 0; }
static int writeFileSilent(const char *path, const char *value) { FILE *f = fopen(path, "w"); if (!f) return -1; if (fprintf(f, "%s", value) < 0) { fclose(f); return -1; } fclose(f); return 0; }
static int fileExists(const char *path) { struct stat st; return (stat(path, &st) == 0); }

// Sysfsに依存していた駆動関数を WiringPi に置き換え
static int gpio_init_and_on(void) {
    // WiringPi BCM モードでピンをセットアップし、HIGHにする
...
static void gpio_off_and_unexport(void) {
    ...
    // WiringPiSetupGpio が呼ばれていれば、プログラム終了時に自動クリーンアップされる
}
...
static int i2c_read_len(uint8_t reg, uint8_t *buf, int len) {
    if (write(i2c_fd, &reg, 1) != 1) { perror("i2c_read_len:write(reg)");
return -1; }
...
void process_received_data(const char *received_data) {
    char data_copy[MAX_DATA_LEN]; strncpy(data_copy, received_data, MAX_DATA_LEN);
data_copy[MAX_DATA_LEN - 1] = '\0';
    char *child_addr = strtok(data_copy, ","); char *surface = strtok(NULL, ",");
if (child_addr && surface) {
        int child_key = get_key_by_addr(child_addr);
        int child_index = get_index_by_addr(child_addr);

        // レポートログがまだ出力されていない場合のみ出力
        if (child_index != -1 && g_report_log_printed[child_index] == 0) {
            ...
            g_report_log_printed[child_index] = 1; // ログ出力フラグをセット
        }
...
if (g_total_children <= 0) { pthread_mutex_unlock(&ready_mutex); return; }
    if (g_parent_addr[0] != '\0' && strcmp(child_addr, g_parent_addr) == 0) { pthread_mutex_unlock(&ready_mutex);
return; }
    int idx = get_index_by_addr(child_addr);
    if (idx < 0 || idx >= MAX_NODES) { pthread_mutex_unlock(&ready_mutex); return;
}
    if (!g_child_ready[idx]) {
        g_child_ready[idx] = 1; g_ready_count++;
...
// ==========================================================
// 親機のホールセンサー常時監視スレッド (時間分割フェーズに対応)
// ==========================================================
void *parent_sensor_monitor(void *arg) {
    (void)arg; printf("[SENSOR] 親機センサー監視スレッド開始。\n");
    static char last_surface[16] = "";
static int same_count = 0;

    while (1) {
        if (g_sensor_phase != 1) { usleep(100000);
continue; }

        // 親機の電磁石がONの場合はセンサー監視をスキップ
...
for (int ch = 1; ch < NUM_CH; ch++) { window[ch] = read_adc_voltage(ch);
        ...
        double maxDiff = 0.0; int maxCh = -1;
...
if (heading_north >= 225.0f && heading_north < 315.0f) rotation_index = 3;
                    ...
                    int initial_index = -1;
if (maxCh == 2) initial_index = 0; else if (maxCh == 3) initial_index = 1;
if (maxCh == 4) initial_index = 2; else if (maxCh == 5) initial_index = 3;
if (initial_index != -1) {
                        int mapped_index = (initial_index + rotation_index) % 4;
const char* final_names[] = {"FRONT", "LEFT", "BACK", "RIGHT"}; detected_surface = final_names[mapped_index];
}
                }
...
strncpy(current_surface, detected_surface, sizeof(current_surface));
                ...
                if (strcmp(last_surface, current_surface) == 0) { same_count++; } else { strncpy(last_surface, current_surface, sizeof(last_surface));
last_surface[sizeof(last_surface)-1] = '\0'; same_count = 1; }

                if (same_count >= STABLE_COUNT_REQUIRED) {
                    // ログ出力を子機と同様の形式に統一
                    printf("\n[EVENT CONFIRMED] CH%d detected -> Surface: **%s** (Internal Detection)\n", maxCh, current_surface);
                    
                    same_count = 0; last_surface[0] = '\0';
                }
            }
        }
        usleep(10000);
...
printf("[COMM] BLE親機サーバー起動。子機データ受信待機中...\n");
    ...
    int dev_id = hci_get_route(NULL); if (dev_id < 0) { perror("hci_get_route"); return NULL;
}
    int sock = hci_open_dev(dev_id); if (sock < 0) { perror("hci_open_dev"); return NULL;
}

    struct hci_filter nf; hci_filter_clear(&nf); hci_filter_set_ptype(HCI_EVENT_PKT, &nf);
    hci_filter_set_event(EVT_LE_META_EVENT, &nf); setsockopt(sock, SOL_HCI, HCI_FILTER, &nf, sizeof(nf));
    le_set_scan_parameters_cp scan_params_cp;
memset(&scan_params_cp, 0, sizeof(scan_params_cp));
    scan_params_cp.type = 0x01; scan_params_cp.interval = htobs(0x0010); scan_params_cp.window = htobs(0x0010);
...
hci_send_cmd(sock, OGF_LE_CTL, OCF_LE_SET_SCAN_PARAMETERS, sizeof(scan_params_cp), &scan_params_cp);
    ...
    uint8_t enable = 0x01, filter_dup = 0x00; uint8_t cmd[2] = { enable, filter_dup };
...
uint8_t *ptr = buf + (1 + HCI_EVENT_HDR_SIZE); evt_le_meta_event *meta = (evt_le_meta_event *)ptr;
...
char addr[18]; ba2str(&info->bdaddr, addr); char name[128] = ""; int pos = 0;
...
if (p) {
                    p += strlen("SURFACE:");
char surface[32]; int si = 0;
                    while (*p != '\0' && *p != '|' && si < 31) surface[si++] = *p++;
surface[si] = '\0'; char combined[MAX_DATA_LEN];
...
// ==========================================================
// mag_sequence_thread: 時間分割ロジック適用 (Key昇順)
// ==========================================================
void *mag_sequence_thread(void *arg) {
    (void)arg;
...
pthread_mutex_lock(&mag_mutex);
    ...
    g_parent_mag_active = 0; g_sensor_phase = 0;
sleep((unsigned int)COOLDOWN_SEC);

    // P-SCAN前にレポートログ出力フラグをリセット
...
int key = g_nodes[idx].key;
        ...
        pthread_mutex_lock(&mag_mutex); strncpy(current_mag_target, target_addr, 18); pthread_mutex_unlock(&mag_mutex);
// A. C-MT: MT通知の送信に専念 (10秒)
        printf("\n[PHASE START] 子機(Key=%d) MT通知フェーズ (C-MT) (%d秒)\n", key, C_MT_TIME_SEC);
uint8_t adv_data[31]; memset(adv_data, 0, sizeof(adv_data)); int len = 0;
...
adv_cp.length = (uint8_t)len; memcpy(adv_cp.data, adv_data, len);
...
hci_send_cmd(sock_adv, OGF_LE_CTL, OCF_LE_SET_ADVERTISE_ENABLE, 1, &enable);
        ...
        // C-ACT前にレポートログ出力フラグをリセット
        memset(g_report_log_printed, 0, sizeof(g_report_log_printed));

// B. C-ACT: 親機センサー検知専念 (20秒)
...
g_sensor_phase = 1; 
        ...
        pthread_mutex_lock(&mag_mutex); current_mag_target[0] = '\0';
pthread_mutex_unlock(&mag_mutex);
        sleep((unsigned int)COOLDOWN_SEC);
    }

    // --- シーケンス完了 ---
    pthread_mutex_lock(&mag_mutex); current_mag_target[0] = '\0'; g_sensor_phase = 0;
pthread_mutex_unlock(&mag_mutex);
    close(sock_adv);
    printf("[MAG] 全シーケンス完了。\n");
...
const char *surf = g_reports[r].surface;
                    ...
                    for (int j = 0; j < g_node_count; j++) {
                        if (strcmp(g_nodes[j].addr, det_addr) == 0) { det_idx = j;
break; }
                    }
                    ...
                    if (det_idx != -1 && g_nodes[det_idx].x == -1) {
                        ...
                        // 座標決定ロジックの正負反転
if (strcmp(surf, "FRONT") == 0)      ny -= 1; // FRONT -> -Y
else if (strcmp(surf, "BACK") == 0)  ny += 1; // BACK  -> +Y
else if (strcmp(surf, "LEFT") == 0)  nx += 1; // LEFT  -> +X
else if (strcmp(surf, "RIGHT") == 0) nx -= 1; // RIGHT -> -X
else if (strcmp(surf, "TOP") == 0)   nz -= 1; // TOP   -> -Z
else if (strcmp(surf, "BOTTOM") == 0)nz += 1; // BOTTOM-> +Z
...
for (int i = 0; i < g_node_count; i++) {
    ...
    // マッピング計算と書き出しが完了した後にレポートデータをクリア
    pthread_mutex_lock(&report_mutex);
...
adv_params_cp.max_interval = htobs(interval);
    ...
    uint8_t adv_data[31]; struct { uint8_t length;
uint8_t data[31]; } __attribute__((packed)) adv_cp;
    uint8_t enable;

    for (int i = 0; i < g_node_count; i++) {
        int key = g_nodes[i].key;
int x = g_nodes[i].x; int y = g_nodes[i].y; int z = g_nodes[i].z;
        char addr_hex[13]; compress_addr_to_hex12(g_nodes[i].addr, addr_hex, sizeof(addr_hex));
        char name_field[32];
snprintf(name_field, sizeof(name_field), "MK:%d:%s:%d,%d,%d", key, addr_hex, x, y, z);

        int len = 0; memset(adv_data, 0, sizeof(adv_data));
        adv_data[len++] = 2;
adv_data[len++] = 0x01; adv_data[len++] = 0x06;
        int name_len = (int)strlen(name_field); if (name_len > 26) name_len = 26;
adv_data[len++] = (uint8_t)(name_len + 1); adv_data[len++] = 0x09;
        memcpy(&adv_data[len], name_field, name_len); len += name_len;
        adv_cp.length = (uint8_t)len; memcpy(adv_cp.data, adv_data, len);
hci_send_cmd(sock, OGF_LE_CTL, OCF_LE_SET_ADVERTISING_DATA, len + 1, &adv_cp);
        enable = 0x01; hci_send_cmd(sock, OGF_LE_CTL, OCF_LE_SET_ADVERTISE_ENABLE, 1, &enable);
printf("[COMM] MAP FRAME ADV: %s (5 seconds)\n", name_field); sleep(5);

        enable = 0x00; hci_send_cmd(sock, OGF_LE_CTL, OCF_LE_SET_ADVERTISE_ENABLE, 1, &enable);
        usleep(200000);
}

    char name_field[32]; snprintf(name_field, sizeof(name_field), "MAP_END:%s", map_result == 0 ? "SUCCESS" : "FAILURE");
...
return 1;
    ...
    strncpy(g_parent_addr, parent_addr, 18); g_parent_addr[17] = '\0';
printf("\n=== PARENT PROGRAM START ===\n");

    // ★ 修正 1: WiringPi の初期化を BCM モードに統一 (GPIOの不安定さを解消)
    ...
    if (wiringPiSPISetup(SPI_CH, SPI_SPEED) < 0) {
         fprintf(stderr, "FATAL ERROR: wiringPiSPISetup failed. Check SPI setup (SPI_CH=%d, SPEED=%d).\n", SPI_CH, SPI_SPEED);
return 2;
    }
    i2c_fd = i2c_open();
    if (i2c_fd < 0 || bno055_init_ndof() < 0) return 3;

    // GPIO pin number for magnet is BCM_GPIO_PIN (20)
...
g_ready_count = 0; g_total_children = 0;
        for (int i = 0; i < g_node_count; i++) {
...
void *thread_result;
        pthread_join(mag_thread, &thread_result);
