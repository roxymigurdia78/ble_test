...
typedef struct __attribute__((packed)) {
    uint16_t company_id;
    uint16_t seq;
    uint8_t  song_id; // ここに 0xFF が入ったら停止命令
    int8_t   origin_x; 
    int8_t   origin_y; 
    int8_t   origin_z; 
} EffectPayload;
...
// ---------------------------------------------------------------------------
int main() {
    printf("=== Effect Main (Global Stop Version) ===\n");
    ...
    softPwmCreate(PIN_LED_R, 100, 100); softPwmCreate(PIN_LED_G, 100, 100); softPwmCreate(PIN_LED_B, 100, 100);
