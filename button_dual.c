// ================================================
// button_dual.c (長押し対応版)
// - 短押し: ネットワークモード (Exit Code 0/1)
// - 長押し(2秒): ソロモード (Exit Code 99)
// ================================================
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
#define LONG_PRESS_MS        2000 // 長押し判定時間 (2000ms = 2秒)
...
void led_set_blue() { led_all_off(); digitalWrite(PIN_LED_BLUE, 1); }
void led_set_red() { led_all_off(); digitalWrite(PIN_LED_RED, 1); }
void led_set_green() { led_all_off(); digitalWrite(PIN_LED_GREEN, 1); }
// ソロモード通知用（赤+青=マゼンタ）
void led_set_magenta() { led_all_off(); digitalWrite(PIN_LED_RED, 1); digitalWrite(PIN_LED_BLUE, 1); }

void led_blink(int pin, int interval_ms) {
    led_all_off();
    for (int i = 0; i < 3; i++) {
        digitalWrite(pin, 1); usleep(interval_ms * 1000);
        digitalWrite(pin, 0); usleep(interval_ms * 1000);
    }
}
...
int get_start_flag(void) {
    int v; pthread_mutex_lock(&start_mutex); v = start_flag; pthread_mutex_unlock(&start_mutex); return v;
}
...
void set_start_flag(void) {
    ...
    if (!start_flag) {
        start_flag = 1;
        // ソロモードでなければ赤にする（ソロの場合はマゼンタになっているため上書きしない）
        if (!mode_solo) {
...
void save_device_list_for_parent(const char *filename) {
    ...
    if (!fp) { perror("[ERROR] Failed to open key list file"); return; }
    fprintf(fp, "%d %s\n", my_key, my_addr);
    pthread_mutex_lock(&detected_mutex);
    for (int i = 0; i < detected_count; i++) {
        if (detected_devices[i].key >= 0) fprintf(fp, "%d %s\n", detected_devices[i].key, detected_devices[i].addr);
    }
    pthread_mutex_unlock(&detected_mutex);
    fclose(fp);
    printf("[INFO] Device list auto-saved to %s\n", filename);
}
...
void *advertise_thread(void *arg) {
    ...
    while (1) {
        ...
        char paddr_copy[18]; strncpy(paddr_copy, perceived_parent_addr_last3, sizeof(paddr_copy));
        ...
        adv_data[len++] = name_len + 1; adv_data[len++] = 0x09;
        memcpy(&adv_data[len], name_field, name_len); len += name_len;
        adv_data_cp_struct.length = len; memcpy(adv_data_cp_struct.data, adv_data, len);
        
        hci_send_cmd(global_adv_sock, OGF_LE_CTL, OCF_LE_SET_ADVERTISING_DATA, len + 1, &adv_data_cp_struct);
        uint8_t enable = 0x01; hci_send_cmd(global_adv_sock, OGF_LE_CTL, OCF_LE_SET_ADVERTISE_ENABLE, 1, &enable);
...
void *scan_thread(void *arg) {
    ...
    hci_filter_set_ptype(HCI_EVENT_PKT, &nf); hci_filter_set_event(EVT_LE_META_EVENT, &nf);
    ...
    printf("=== Waiting for button press (Short=Net, Long=Solo) or remote |S ===\n");
    led_set_blue(); 
    ...
    while (!get_start_flag()) {
        ...
        for (int i = 0; i < reports; i++) {
            ...
            char name[128] = ""; int key = -1; int pos = 0;
    ...
    // ★ ソロモードなら選挙せずに終了
    if (mode_solo) {
        printf("[INFO] Solo mode selected. Stopping scan.\n");
    ...
    int smallest_key = my_key; char smallest_addr_full[18] = "(self)";
    pthread_mutex_lock(&detected_mutex);
    for (int i = 0; i < detected_count; i++) {
    ...
    char smallest_addr_last3[18]; get_last_three(smallest_addr_full, smallest_addr_last3, 18); 
    ...
    VerEntry ver_list[MAX_DEVS]; int ver_count = 0; time_t ver_start = time(NULL);

    while (difftime(time(NULL), ver_start) < PARENT_EXCHANGE_SEC) {
        int len = read(scan_sock, buf, sizeof(buf));
        if (len < 0) continue; if (len < (1 + HCI_EVENT_HDR_SIZE)) continue;
        evt_le_meta_event *meta = (evt_le_meta_event *)(buf + (1 + HCI_EVENT_HDR_SIZE));
        if (meta->subevent != EVT_LE_ADVERTISING_REPORT) continue;
        uint8_t reports = meta->data[0]; uint8_t *offset = meta->data + 1;
        for (int i = 0; i < reports; i++) {
            ...
            char rep_parent[18] = ""; extract_parent_tag_from_name(name, rep_parent, 18);
            if (rep_parent[0] != '\0') {
                int found = 0;
                for(int k=0; k<ver_count; k++) {
                    if(strcmp(ver_list[k].addr, addr) == 0) { strncpy(ver_list[k].rep_parent, rep_parent, 18); found=1; break; }
                }
                if(!found && ver_count < MAX_DEVS) {
                    strncpy(ver_list[ver_count].addr, addr, 18); strncpy(ver_list[ver_count].rep_parent, rep_parent, 18); ver_count++;
                }
            }
            offset = (uint8_t *)info + sizeof(*info) + info->length;
        }
    ...
    pthread_mutex_lock(&detected_mutex); int total = detected_count + 1; pthread_mutex_unlock(&detected_mutex);
    printf("\nReceived reports from %d/%d devices.\n", ver_count, total-1);
    
    int all_match = 0;
    if (total == 1) all_match = 1;
    else if (ver_count > 0) {
        int match_count = 0;
        for(int i=0; i<ver_count; i++) if(strcmp(ver_list[i].rep_parent, perceived_parent_addr_last3) == 0) match_count++;
        if (match_count > 0) all_match = 1; 
    }

    if (all_match) {
        pthread_mutex_lock(&parent_mutex); strncpy(final_parent_addr_full, perceived_parent_addr_full, 32); pthread_mutex_unlock(&parent_mutex);
        final_success = 1; printf("🎉 CONFIRMED Parent: %s\n", final_parent_addr_full);
    } else {
        final_success = 0; printf("⚠️ Election failed.\n");
    }
    pthread_mutex_lock(&parent_mutex); parent_phase = 0; pthread_mutex_unlock(&parent_mutex);
    return NULL;
}
...
void *button_thread(void *arg) {
    while (!get_start_flag()) {
        if (digitalRead(PIN_BUTTON) == 0) { // ボタン押下
            ...
            while (digitalRead(PIN_BUTTON) == 0) {
                ...
                if (current_ms - start_ms > LONG_PRESS_MS) {
                    // 長押し確定 -> 色を変えてユーザーに通知 (マゼンタ)
                    if (!long_press_detected) {
                        printf("[BTN] Long press detected! Switching to SOLO MODE.\n");
                        led_set_magenta(); 
            ...
            if (long_press_detected) {
                mode_solo = 1;
                set_start_flag(); // ループを抜けるためにフラグを立てる
            } else {
                // 短押し -> 通常モード
                printf("[BTN] Short press. Starting NETWORK MODE.\n");
                mode_solo = 0;
                set_start_flag();
            }
            break; 
...
int main(int argc, char *argv[]) {
    ...
    bdaddr_t bdaddr; hci_read_bd_addr(global_adv_sock, &bdaddr, 1000); ba2str(&bdaddr, my_addr);
    srand(time(NULL) ^ getpid()); my_key = (rand() % 256); 
    ...
    pthread_cancel(t_adv); pthread_cancel(t_btn);
    uint8_t d=0; hci_send_cmd(global_adv_sock, OGF_LE_CTL, OCF_LE_SET_ADVERTISE_ENABLE, 1, &d);
    close(global_adv_sock);

    // ★ ソロモードの場合、99を返して終了
    if (mode_solo) {
        printf(">>> SOLO MODE ACTIVATED (Exit Code 99) <<<\n");
        led_blink(PIN_LED_BLUE, 100); // 決定時の短い点滅
        led_all_off();
        return 99; // ソロモード用終了コード
    }
    ...
    if (final_success) {
        ...
        int exit_code = 0;
        if (WIFEXITED(sys_ret)) exit_code = WEXITSTATUS(sys_ret); else exit_code = -1; 
